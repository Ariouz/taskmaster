...
void ProcessManager::monitor() {
    for (auto& [pid, process] : _processes) {
        ...
        if (result == 0) {
            ...
            if (process.getStatus() == Status::STARTING && diff >= process.getConfig().starttime) process.setStatus(Status::RUNNING);
        }
...
void ProcessManager::handleAutoRestart(Process& process, int code, bool killedBySignal) {
    if (process.shouldRestart(code, killedBySignal)) {
        Logger::info("Restarting " + process.getName() + ": " + std::to_string(process.getRetries()) + "/" + std::to_string(process.getConfig().startretries));
